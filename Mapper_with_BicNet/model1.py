...
# add
        ...
        self.post_dense_c = DenseNet(HIDDEN_DIM + self.s_dim, HIDDEN_DIM // 2, 1, output_activation=None)
# ----------------
        self.fc3 = nn.Linear(64, 32)
        self.fc4 = nn.Linear(32, 9)
    ...
    def forward(self, state_img, state_val):
        ...
        img_list=[ torch.tensor([img]).to(self.device) for img in img_list]
        x = F.relu(self.fc1(x))
        y = F.relu(self.fc_val(state_val))
        x = torch.cat((x, y), dim=1)
        x = F.relu(self.fc2(x))
#-------------------------------
        ...
        v=self.post_dense_c(x_c)
        v= v.view(-1, self.n_agents, 1)
        ...
        # print('log_probs_list',log_probs_list,
        #       '\n m.log',m.log_prob(origin_actions))
        for i in origin_actions.cpu().numpy()[0]:
            action_list.append(idx_to_act[i])
            attion_actions_list.append(cast([insert_action(i)]))
        # add--------------------
        return attion_actions_list, action_list, actions_probs, m.log_prob(origin_actions), m.entropy(),v

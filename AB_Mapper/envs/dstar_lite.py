...
#                raise Exception("Fail! Stuck in a loop")
    ...
    def plan(self, pose, goal, obs = None, debug = False):
        ...
        if self.next_step is None: # first run compute shortest path or when last run cannot find solution
            #print("no path")
            if obs is not None:
        ...
        self.next_step = np.array(self.lowest_cost_neighbour(self.position))
        return self.next_step#self.path
    ...
    def get_path(self):
        ...
        if self.goal == self.position: return []
        else:
            self.path = [self.next_step]
            cur = self.lowest_cost_neighbour(self.position)
            for i in range(2):
                if (cur == self.goal): break
                #print("cur",cur)
                cur = self.lowest_cost_neighbour(cur)#cur = self.back_pointers[cur]
                self.path.append(np.array(cur))
        return self.path

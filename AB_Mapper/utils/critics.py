...
class AttentionCritic(nn.Module):
    ...
    """
    def __init__(self, sa_sizes, hidden_dim=512, norm_in=False, attend_heads=1):
        """
        Inputs:
    ...
    def scale_shared_grads(self):
        ...
        for p in self.shared_parameters():
            ...
            # p.grad.mul_(1. / self.nagents)

    def forward(self, inps, agent_index, agents=None, return_q=True, return_all_q=False,
        ...
        all_head_keys = [[k_ext(enc) for enc in sa_encodings] for k_ext in self.key_extractors]# every key_extractor encoder all sa_encoder
        ...
        all_head_selectors = [[sel_ext(enc) for i, enc in enumerate(s_encodings) if i in agents]
                              for sel_ext in self.selector_extractors]#q_i are agent_i's state encodings
        ...
        for curr_head_keys, curr_head_values, curr_head_selectors in zip(
            ...
            #     print('selector.view(selector.shape[0], 1, -1)',selector.view(selector.shape[0], 1, -1))
            #     # calculate attention across agents
            #     attend_logits = torch.matmul(selector.view(selector.shape[0], 1, -1),
            #                                  torch.stack(keys).permute(1, 2, 0))#q_i*K_j
            #     # scale dot-products by size of key (from Attention is All You Need)
            #     scaled_attend_logits = attend_logits / np.sqrt(keys[0].shape[1])#q_i*K_j/sqrt(d_k)
            #     attend_weights = F.softmax(scaled_attend_logits, dim=2)#w_j
            #     other_values = (torch.stack(values).permute(1, 2, 0) *
            #                     attend_weights).sum(dim=2) #x_i=sum(w_j*v_j)
        ...
        # calculate Q per agent
            for i, a_i, selector in zip(range(len(agents)), agents, curr_head_selectors):
                keys = [k for j, k in enumerate(curr_head_keys) if j in agent_index[a_i]]
                values = [v for j, v in enumerate(curr_head_values) if j in agent_index[a_i]]
                # calculate attention across agents
                # print('selector.view(selector.shape[0], 1, -1)', selector.view(selector.shape[0], 1, -1))
                attend_logits = torch.matmul(selector.view(selector.shape[0], 1, -1),
                                             torch.stack(keys).permute(1, 2, 0))#q_i*K_j

...
class AttentionSAC(object):
    ...
    def update_my_critic(self, sample, next_input_img_list,next_input_val_list, agent_list,
        ...
        for pi, ob, iv in zip(agent_list, next_input_img_list, next_input_val_list):
            ...
            _, _, curr_next_ac, curr_next_log_pi, _,_ = pi.act(ob,iv)
        ...
        next_qs = self.target_critic(trgt_critic_in, attention_index_tar, return_q=True, return_all_q=False)
        critic_rets = self.critic(critic_in, attention_index, return_q=True, return_all_q=False, regularize=True)
        ...
        q_loss.backward()
        # torch_utils.clip_grad_norm_(self.critic.parameters(), int(q_loss.item()//2))# 既然在BP过程中会产生梯度消失/爆炸（就是偏导无限接近0，导致长时记忆无法更新），                                                # 那么最简单粗暴的方法，设定阈值，当梯度小于/大于阈值时，更新的梯度为阈值，如下图所示：
